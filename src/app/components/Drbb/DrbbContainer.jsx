...
const DrbbContainer = ({ drbbResults }) => {
  const { works, totalWorks, researchNowQueryString } = drbbResults;
    ...
    if (hasWorks) {
      return [
        <ul key="drbb-scc-results-list" className="drbb-list">
          {works.map((work) => (
            <DrbbResult key={work.id} work={work} />
          ))}
        </ul>,
        <Link
        ...
        >
          See {totalWorks.toLocaleString()} result{totalWorks === 1 ? "" : "s"}{" "}
          from Digital Research Books Beta
        </Link>,
      ];
    }
  ...
  return (
    <div className="drbb-container">
      <h3 className="drbb-main-header">
        {hasWorks ? "Results from" : "No results found from"} Digital Research
        Books Beta
      </h3>
      <p className="drbb-description">
        Digital books for research from multiple sources world wide- all free to
        read, download, and keep. No Library Card is Required.&nbsp;
        <span>
          <a
            className="link"
            target="_blank"
