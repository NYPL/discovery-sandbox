...
import { trackDiscovery, getElectronicResources } from '../../utils/utils';
...
export const getBibTitle = (bib) => {
  if (!bib.titleDisplay || !bib.titleDisplay.length) {
    const author =
      bib.creatorLiteral && bib.creatorLiteral.length
...
export const getYearDisplay = (bib) => {
  ...
  if (dateStartYear && dateEndYear) {
    return (
      <li className='nypl-results-date'>
...
const ResultsList = (
  { results, subjectHeadingShow, searchKeywords },
  { router },
) => {
  ...
  const generateBibLi = (bib, i) => {
    ...
    const materialType =
      result && result.materialType && result.materialType[0]
        ? result.materialType[0].prefLabel
        : null;
    const yearPublished = getYearDisplay(result);
    const publicationStatement =
      result.publicationStatement && result.publicationStatement.length
        ? result.publicationStatement[0]
        : '';
    const items = (result.checkInItems || []).concat(
      ItemSorter.sortItems(LibraryItem.getItems(result)),
    );
    ...
    const itemMessage = `${itemCount} ${resourceType}${
      itemCount !== 1 ? 's' : ''
    }`;
    return (
      <li
        ...
        <Card className='search-results-bib-card'>
          ...
          <CardContent className='nypl-results-item-description'>
            <ul>
              ...
              <li className='nypl-results-publication'>
                {publicationStatement}
              </li>
              {yearPublished}
              {totalItems > 0 ? (
                <li className='nypl-results-info'>{itemMessage}</li>
              ) : (
                ''
              )}
            </ul>
                  ...
                  <CardActions className='results-items-element'>
                    ...
                    >
                      <Text isBold size='caption'>
                        {`View All ${totalPhysicalItems} Items`}{' '}
                        <RightWedgeIcon />
                      </Text>
                    </Link>
                  </CardActions>
  ...
  return (
    <ul
      ...
      className={`nypl-results-list${loading ? ' hide-results-list' : ''}${
