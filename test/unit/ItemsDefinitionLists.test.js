  ...
  describe('rendering items', () => {
    describe('list structure', () => {
      ...
      it('should have a div with correct props', () => {
        ...
        expect(root.prop('className')).to.equal(
          'items-definition-list results-items-element',
        );
      });
      ...
      it('should have a div with correct props for each item', () => {
        const divs = component
          .first()
          .children()
          .find('div')
          .map((div) => [div.key(), div.prop('id')]);
        expect(divs).to.deep.equal(
          items
            .slice(4)
            .map((item) => [`item-${item.id}-div`, `item-${item.id}-div`]),
        );
      });
      ...
      it('should have a dl with correct id and key for each item', () => {
        ...
        expect(dls).to.deep.equal(
          items
            .slice(4)
            .map((item) => [`item-${item.id}-dl`, `item-${item.id}-dl`]),
        );
      });

      it('should have correct number of dt and dd cells with correct props', () => {
        const dlcells = component
          .first()
          .children()
          .find('div')
          .map((div) => div.find('dl'))
        ...
        expect(dlcells).to.deep.equal(
          items.slice(4).map((item) => [
            ...
            [
              'dt',
              `item-${item.id}-callnumber-dt`,
              `item-${item.id}-callnumber-dt`,
            ],
            [
              'dd',
              `item-${item.id}-callnumber-dd`,
              `item-${item.id}-callnumber-dd`,
            ],
            [
              'dt',
              `item-${item.id}-location-dt`,
              `item-${item.id}-location-dt`,
            ],
            [
              'dd',
              `item-${item.id}-location-dd`,
              `item-${item.id}-location-dd`,
            ],
          ]),
        );
      ...
      it('should render Format dt', () => {
        expect(component.find('dl').at(0).find('dt').at(0).text()).to.equal(
          'Format',
        );
      });
      it('should render item format when present', () => {
        expect(component.find('dl').at(0).find('dd').at(0).text()).to.equal(
          'Text',
        );
      });
      ...
      it('should render Call Number dt', () => {
        expect(component.find('dl').at(0).find('dt').at(1).text()).to.equal(
          'Call Number',
        );
      });

      it('should render call number when present', () => {
        expect(component.find('dl').at(0).find('dd').at(1).text()).to.equal(
          'ABCDE1234',
        );
      });
      ...
      it('should render item location dt', () => {
        expect(component.find('dl').at(0).find('dt').at(2).text()).to.equal(
          'Item Location',
        );
      });
      ...
      it('should render item location link when url present', () => {
        expect(
          component.find('dl').at(2).find('dd').at(2).find('a').length,
        ).to.equal(1);
        expect(
          component.find('dl').at(2).find('dd').at(2).find('a').text(),
        ).to.equal('SASB');
        expect(
          component.find('dl').at(2).find('dd').at(2).find('a').prop('href'),
        ).to.equal('www.fake.com');
        expect(
          component
            .find('dl')
            .at(2)
            ...
            .find('a')
            .prop('className'),
        ).to.equal('itemLocationLink');
      });

      it('should render item location text when location present', () => {
        expect(
          component.find('dl').at(3).find('dd').at(2).find('a').length,
        ).to.equal(0);
        expect(component.find('dl').at(3).find('dd').at(2).text()).to.equal(
          'SASB',
        );
      });
      ...
      it('should default to blank location', () => {
        expect(
          component.find('dl').at(4).find('dd').at(2).find('a').length,
        ).to.equal(0);
