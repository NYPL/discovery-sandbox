...
describe('BibDetails', () => {
  ...
  describe('Top fields', () => {
    ...
    it('should display titles, authors', () => {
      ...
      expect(component.find('dd').at(1).text()).to.equal(
  ...
  describe('Bottom fields', () => {
    ...
    [
      ...
      xit(`should display publication, extent, subjects, shelfMark, and other identifiers [${spec.description}]`, () => {
        ...
        expect(component.find('dd').at(3).text()).to.equal(
          bibs[0].note[0].prefLabel,
        );
        expect(component.find('dd').at(4).text()).to.equal(
          bibs[0].shelfMark[0],
        );
        // Isbn:
        ...
        expect(component.find('dd').at(5).find('li').at(0).text()).to.equal(
          isbn['@value'],
        );
        ...
        if (typeof bibs[0].identifier[0] === 'string') {
          expect(component.find('dd').at(5).find('li').at(1).text()).to.equal(
  ...
  describe('Subject headings', () => {
    ...
    xit('should render proper texts and link(s) for each subject heading', () => {
      ...
      expectSubjectLiterals.forEach((subjectLiteral, ind) => {
        expect(component.find('dd').at(2).find('li').at(ind).text()).to.equal(
          subjectLiteral.text,
        );
