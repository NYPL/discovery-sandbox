...
import WrappedHoldConfirmation, {
  HoldConfirmation,
} from './../../src/app/pages/HoldConfirmation';
...
describe('HoldConfirmation', () => {
  describe('After being rendered, <HoldConfirmation>', () => {
    ...
    before(() => {
      ...
      component = mountTestRender(<HoldConfirmation location={location} />, {
        store: makeTestStore(),
      });
    });
  ...
  describe("If the patron is logged in but the App doesn't get valid data,<HoldConfirmation>", () => {
    ...
    it('should render the error message.', () => {
      ...
      expect(expectedHTML.test(main.html())).to.equal(true);
    });
  });
  ...
  describe('If the patron is logged in and the App receives valid data, <HoldConfirmation>', () => {
    ...
    it("should display the layout of request confirmation page's contents.", () => {
      ...
      expect(main.find('#physical-delivery').text()).to.equal(
        'Physical Delivery',
      );
      expect(main.find('#electronic-delivery').text()).to.equal(
        'Electronic Delivery',
      );
      expect(
        main.contains(<h3 id='physical-delivery'>Physical Delivery</h3>),
      ).to.equal(true);
      expect(
        main.contains(<h3 id='electronic-delivery'>Electronic Delivery</h3>),
      ).to.equal(true);
    ...
    it('should have the link back to homepage.', () => {
      ...
      expect(main.find('#start-new-search').at(1).text()).to.equal(
        'Start a new search',
      );
    });
  });
  ...
  describe(
    'If the patron is logged in and the App receives valid data but no delivery ' +
      'location label',
    () => {
      const location = {
        query: {
  ...
  describe('If the App does not receive valid pick up location data', () => {
    ...
    it('should render the default error message.', () => {
      ...
      expect(main.find('#delivery-location')).to.have.length(1);
      expect(main.find('#delivery-location').text()).to.equal(
        'The item will be delivered to: please email us or call 917-ASK-NYPL (917-275-6975) for your delivery location.',
      );
  ...
  describe('If the patron gets here from a search result page, <HoldConfirmation>', () => {
    ...
    it('should have the link back to search result.', () => {
      ...
      expect(main.find('#go-back-search-results').at(1).text()).to.equal(
  ...
  describe('If the patron does not get here from a classic catalog, <HoldConfirmation>', () => {
    ...
    before(() => {
      renderBackToClassicLink = sinon.spy(
        HoldConfirmation.prototype,
  ...
  describe('If the patron gets here from a classic catalog search result page, <HoldConfirmation>', () => {
    ...
    const location = {
      query: {
        ...
        searchKeywords: 'Bryant',
        fromUrl:
          'https://catalog.nypl.org/search~S1/?searchtype=X&searcharg=bryant&' +
    ...
    it('should have the link back to the classic catalog homepage and search result page', () => {
      ...
      expect(main.find('#go-back-catalog a').at(0).text()).to.equal(
        'Go back to your search results',
      );
      expect(main.find('#go-back-catalog a').at(0).prop('href')).to.equal(
        'https://catalog.nypl.org/search~S1/?searchtype=X&searcharg=bryant&' +
          'searchscope=1&sortdropdown=-&SORT=DZ&extended=0&SUBMIT=Search&searchlimits' +
          '=&searchorigarg=Xbryant%26SORT%3DD',
      );

      expect(main.find('#go-back-catalog a').at(1).text()).to.equal(
        'start a new search',
      );
      expect(main.find('#go-back-catalog a').at(1).prop('href')).to.equal(
    ...
    it('should have the link to shared collection catalog.', () => {
      ...
      expect(main.find('#go-to-research-catalog').text()).to.equal(
        ' You may also try your search in our Research Catalog.',
      );
    });
  });
  ...
  describe('If there are eligibility errors', () => {
    it('should render an error message with specific errors when available', () => {
      const location = {
        query: {
          errorStatus: 'eligibility',
          errorMessage: '{"expired":true,"blocked":true,"moneyOwed":true}',
        },
      };
      ...
      expect(
        text.includes('There is a problem with your library account'),
      ).to.equal(true);
      expect(text.includes('Your fines have exceeded the limit')).to.equal(
        true,
      );
    });
    it('should render a default error message when no specific errors are available', () => {
      const location = {
        query: { errorStatus: 'eligibility', errorMessage: '{}' },
      };
      const component = mountTestRender(
        <WrappedHoldConfirmation location={location} />,
        {
