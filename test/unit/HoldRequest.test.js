...
describe('HoldRequest', () => {
  ...
  before(() => {
    ...
    mock
      .onGet(
        '/research/collections/shared-collection-catalog/api/patronEligibility',
      )
      .reply(200, { eligibility: true });
  });
  ...
  describe('If the patron is logged in and the App receives invalid item data, <HoldRequest>', () => {
    ...
    it('should display the error message.', () => {
      ...
      expect(h2.text()).to.equal(
        'This item cannot be requested at this time. Please try again later or contact 917-ASK-NYPL (917-275-6975).',
      );
    });
  });
    ...
    () => {
      ...
      it(
        ...
        () => {
          setImmediate(() => {
            ...
            expect(
              fieldset.find('label').at(0).find('input').props().type,
            ).to.equal('radio');
            expect(
              fieldset.find('label').at(1).find('input').props().type,
            ).to.equal('radio');
            expect(
              fieldset.find('label').at(2).find('input').props().type,
            ).to.equal('radio');
            expect(
              fieldset.find('label').at(0).find('input').props().checked,
            ).to.equal(true);
            expect(
              fieldset.find('label').at(1).find('input').props().checked,
            ).to.equal(false);
            expect(
              fieldset.find('label').at(2).find('input').props().checked,
            ).to.equal(false);
          });
        },
      );
      ...
      it('should display the names and the addresses of the delivery locations.', () => {
        setImmediate(() => {
          ...
          expect(label0.find('.nypl-screenreader-only').text()).to.equal(
            'Send to:',
          );
          expect(label0.find('.nypl-location-name').text()).to.equal(
            'Library for the Performing Arts',
          );
          expect(label0.find('.nypl-location-address').text()).to.equal(
            '40 Lincoln Center Plaza',
          );

          expect(label1.find('.nypl-screenreader-only').text()).to.equal(
            'Send to:',
          );
          expect(label1.find('.nypl-location-name').text()).to.equal(
            'Schomburg Center',
          );
          expect(label1.find('.nypl-location-address').text()).to.equal(
            '515 Malcolm X Boulevard',
          );

          expect(label2.find('.nypl-screenreader-only').text()).to.equal(
            'Send to:',
          );
