...
describe('researchNowUtils', () => {
  ...
  describe('getResearchNowQueryString', () => {
    ...
    it('should handle contributor param', () => {
      expect(getResearchNowQueryString({ contributor: 'Poe' })).to.equal(
        '?query=keyword%3A*%2Cauthor%3APoe&page=1&source=catalog',
      );
    });

    it('should handle title param', () => {
      expect(getResearchNowQueryString({ title: 'Raven' })).to.equal(
        '?query=keyword%3A*%2Ctitle%3ARaven&page=1&source=catalog',
      );
    });

    it('should handle subject param', () => {
      expect(getResearchNowQueryString({ subject: 'corvids' })).to.equal(
        '?query=keyword%3A*%2Csubject%3Acorvids&page=1&source=catalog',
      );
    });
    ...
    it('should combine query params', () => {
      expect(
        ...
        '?query=keyword%3ARaven%2Cauthor%3APoe%2Csubject%3Acorvids&page=1&source=catalog',
      );
    });
  });
});
