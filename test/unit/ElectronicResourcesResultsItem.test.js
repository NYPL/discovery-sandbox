...
describe('ElectronicResourcesResultsItem', () => {
  describe('with no resources', () => {
    it('should render null', () => {
      const component = mount(
        <ElectronicResourcesResultsItem resources={[]} />,
      );
  ...
  describe('with at least one resource', () => {
    ...
    const onClick = (e) => {
      e.preventDefault();
      onClickRecord = 'clicked';
    };
    const component = mount(
      <ElectronicResourcesResultsItem
        resources={[{ url: 'fakeurl', label: 'fakelabel' }]}
        onClick={onClick}
        bibUrl='fakebiburl'
      />,
    );
    it('should have a section with class electronic-resources-search-section', () => {
      ...
      expect(section.at(0).prop('className')).to.equal(
        'electronic-resources-search-section',
      );
    });
    ...
    it('should have an h3 with correct class and text', () => {
      ...
      expect(h3.at(0).prop('className')).to.include(
        'electronic-resources-search-header',
      );
    ...
    it('should have a link', () => {
      ...
      expect(link.at(0).prop('className')).to.include(
        'search-results-single-resource-link',
      );
  ...
  describe('with multiple resources', () => {
    it('should point link to the list of resources', () => {
      const component = mount(
        <ElectronicResourcesResultsItem
          resources={[
            { url: 'fakeurl1', label: 'fakelabel1' },
            { url: 'fakeurl2', label: 'fakelabel2' },
          ]}
          onClick={null}
          bibUrl='fakebiburl'
        />,
      ...
      expect(component.find(Link).at(0).prop('to')).to.equal(
        `fakebiburl#electronic-resources`,
      );
      expect(component.find(Link).at(0).text()).to.include(
