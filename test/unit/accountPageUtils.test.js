...
describe("`convertEncoreUrl`", () => {
  it("should convert a conventionally structured Encore item URL to `discovery-front-end` URL", () => {
    expect(
      convertEncoreUrl(
        "https://browse.nypl.org/iii/encore/record/C__Rb21771946?lang=eng&suite=def"
      )
    ).to.equal(`${appConfig.baseUrl}/bib/b21771946`);
  });

  it("should return passed URL if it does not follow convention of Encore item URL", () => {
    const malformedUrl =
      "https://browse.nypl.org/iii/encore/record/somethingisnotright";
...
describe("`formatPatronExpirationDate`", () => {
  it("should return invalid values unmodified", () => {
    ...
    expect(formatPatronExpirationDate("August 29, 1997")).to.eq(
      "August 29, 1997"
    );
  });
